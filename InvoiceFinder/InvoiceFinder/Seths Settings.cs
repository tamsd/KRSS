...
namespace InvoiceFinder
{
    public partial class MainPage : Form
    {
        ...
        {
            if (ZipFolder.Checked)
            {
                set.setOutputType(2);
            }
            else if (!ZipFolder.Checked)
            {
                ZipFolder.Checked = false;
            }
        }

        private void Add_Location_Click(object sender, EventArgs e)
        {
            set.setOutputPath(Export_Location.Text);
            Export_Location.Clear();
        }
